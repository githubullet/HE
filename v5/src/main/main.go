...
func main() {
	...
	img := bmpRead("/Users/sueyang/Desktop/Code/Single_key_CKKS/v5/src/main/lena50x50.bmp")
	...
	*/
	batchSize := 100 // 每个批次处理100行，可以根据需要调整
	totalRows := img.Bounds().Max.Y

	// 初始化解密后数据的3D存储空间
	decbody := make([][][]float64, totalRows)
	...
	// 3D to 2D
	decbody2d := make([][]float64, totalRows)
	for i := 0; i < totalRows; i++ {
